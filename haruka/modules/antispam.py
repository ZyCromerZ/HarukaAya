...
from haruka import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, STRICT_ANTISPAM
...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in WHITELIST_USERS:
        message.reply_text("OOOH someone's trying to gban a whitelist user! *grabs popcorn*")
        return
    ...
    send_to_list(bot, SUDO_USERS + WHITELIST_USERS,
                 "<b>Global Ban</b>" \
                 "\n#GBAN" \
    ...
    for chat in chats:
        ...
        try:
            ...
            if excp.message in GBAN_ERRORS:
                ...
                send_to_list(bot, SUDO_USERS + WHITELIST_USERS, "Could not gban due to: {}".format(excp.message))
...
def gmute(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in WHITELIST_USERS:
        message.reply_text("OOOH someone's trying to gmute a whitelist user! *grabs popcorn*")
        return
    ...
    for chat in chats:
        ...
        try:
            ...
            if excp.message == "User is an administrator of the chat":
                ...
                send_to_list(bot, SUDO_USERS + WHITELIST_USERS, "Could not gmute due to: {}".format(excp.message))
                sql.ungmute_user(user_id)
                return
        except TelegramError:
            pass

    send_to_list(bot, SUDO_USERS + WHITELIST_USERS, "gmute complete!")
...
def ungmute(bot: Bot, update: Update, args: List[str]):
    ...
    send_to_list(bot, SUDO_USERS + WHITELIST_USERS, "un-gmute complete!")
...
def gkick(bot: Bot, update: Update, args: List[str]):
    ...
    if not user_id:
        ...
        return
    if int(user_id) in SUDO_USERS or int(user_id) in WHITELIST_USERS:
        message.reply_text("OHHH! Someone's trying to gkick a sudo/whitelist user! *Grabs popcorn*")
        return
